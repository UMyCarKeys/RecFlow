    ...
    <Modal open={open} onClose={onClose} title="Upload New Version">
      <form id="upload-version-form" onSubmit={handleSubmit} className="space-y-4">
        <div
          ...
          className="border-2 border-dashed border-line/15 hover:border-accent/50 rounded-xl p-6 text-center cursor-pointer transition-colors"
        >
          {file ? (
            <p className="text-sm text-ink">{file.name} <span className="text-muted">({(file.size / 1024 / 1024).toFixed(1)} MB)</span></p>
          ) : (
            <>
              <p className="text-sm text-muted">Drop your audio file here, or click to browse</p>
              <p className="text-xs text-faint mt-1">MP3, WAV, AIFF, FLAC</p>
            </>
          )}
          <input
            ref={fileRef}
        ...
        {compressing && (
          <div className="space-y-1">
            ...
            <div className="h-1 bg-line/10 rounded-full overflow-hidden">
        ...
        {progress > 0 && progress < 100 && (
          <div id="upload-progress" className="h-1 bg-line/10 rounded-full overflow-hidden">
