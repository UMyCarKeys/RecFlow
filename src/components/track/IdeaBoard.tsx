  ...
  return (
    <div id="idea-board" className="border border-white/8 rounded-xl p-4 space-y-4 bg-surface-2">
      <div>
        ...
        <p className="text-[11px] text-muted/70 mt-0.5">Every idea needs a reference to back it.</p>
      </div>
      ...
      {ideas.length > 0 && (
        <div id="idea-board-list" className="space-y-2">
          {ideas.map((idea, i) => (
            <div key={i} id={`idea-${i}`} className="p-3 rounded-lg bg-surface-3/70 border border-white/8">
              {idea.text && <p className="text-sm text-white/90 leading-relaxed mb-1.5">{idea.text}</p>}
              <div className="flex items-center gap-2">
                ...
                {idea.at && <span className="text-[10px] text-muted flex-shrink-0">{timeAgo(idea.at)}</span>}
                <button onClick={() => remove(i)} className="text-muted hover:text-red-400 transition-colors flex-shrink-0" title="Remove idea">
                  ✕
                </button>
              </div>
            </div>
      ...
      <div id="idea-board-composer" className="space-y-2 pt-1">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="w-32 bg-surface-3 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white placeholder:text-muted focus:outline-none focus:border-accent"
          />
          <input
            value={url}
            ...
            className="flex-1 bg-surface-3 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white placeholder:text-muted focus:outline-none focus:border-accent"
          />
        </div>
