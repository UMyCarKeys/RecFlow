      ...
      <div className="space-y-4">
          ...
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center gap-3">
                ...
                <span className="text-sm text-[#1a1620] flex-1 truncate">{displayName(m.profiles)}</span>
                {isOwner && !memberIsOwner ? (
                  <select
                    value={m.role}
                    onChange={(e) => onUpdateRole(m.id, e.target.value as MemberRole)}
                    className="field-glass border border-black/10 rounded-lg px-2 py-1 text-xs text-[#1a1620] focus:outline-none focus:border-accent"
                  >
                    <option value="contributor">Contributor</option>
                    <option value="viewer">Viewer</option>
                  </select>
                ) : (
                  <span className="text-xs text-[#6b6275] capitalize">{m.role}</span>
                )}
                {isOwner && !memberIsOwner && (
                  <button onClick={() => setRemoving(m)} className="text-[#6b6275] hover:text-red-400 transition-colors" title="Remove">
                    ✕
                  </button>
                )}
              </div>
        ...
        {isOwner && (
          <div className="pt-3 border-t border-black/[0.08] space-y-2">
            <label className="block text-xs text-[#6b6275]">Add a member by username</label>
            <div className="flex gap-2">
              <input
                value={query}
                onChange={(e) => runSearch(e.target.value)}
                placeholder="Search username…"
                className="flex-1 field-glass border border-black/10 rounded-lg px-3 py-2 text-sm text-[#1a1620] placeholder:text-[#9a8fa3] focus:outline-none focus:border-accent"
              />
              <select
                value={addRole}
                onChange={(e) => setAddRole(e.target.value as MemberRole)}
                className="field-glass border border-black/10 rounded-lg px-2 text-sm text-[#1a1620] focus:outline-none focus:border-accent"
              >
                <option value="contributor">Contributor</option>
                <option value="viewer">Viewer</option>
              </select>
              ...
              <div className="space-y-1">
                {results.map((p) => (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => add(p.id)}
                    className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-black/[0.05] transition-colors text-left disabled:opacity-50"
                  >
                    <Avatar src={p.avatar_url} name={displayName(p)} size="sm" />
                    <span className="text-sm text-[#1a1620] flex-1 truncate">{displayName(p)}</span>
                    <span className="text-xs text-accent">Add</span>
                  </button>
                ))}
              </div>
            )}
            {query.trim() && results.length === 0 && <p className="text-xs text-[#6b6275]">No matching users.</p>}
          </div>
        )}
      </div>
