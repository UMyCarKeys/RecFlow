  ...
  return (
    <Modal open={open} onClose={onClose} title="Members">
      <div className="space-y-4">
          ...
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center gap-3">
                ...
                <span className="text-sm text-ink flex-1 truncate">{displayName(m.profiles)}</span>
                {isOwner && !memberIsOwner ? (
                  <select
                    value={m.role}
                    onChange={(e) => onUpdateRole(m.id, e.target.value as MemberRole)}
                    className="field-glass border border-line/10 rounded-lg px-2 py-1 text-xs text-ink focus:outline-none focus:border-accent"
                  >
        ...
        {isOwner && (
          <div className="pt-3 border-t border-line/[0.08] space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 field-glass border border-line/10 rounded-lg px-3 py-2 text-sm text-ink placeholder:text-faint focus:outline-none focus:border-accent"
              />
              <select
                value={addRole}
                onChange={(e) => setAddRole(e.target.value as MemberRole)}
                className="field-glass border border-line/10 rounded-lg px-2 text-sm text-ink focus:outline-none focus:border-accent"
              >
                <option value="contributor">Contributor</option>
                <option value="viewer">Viewer</option>
              </select>
              ...
              <div className="space-y-1">
                {results.map((p) => (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => add(p.id)}
                    className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-line/[0.05] transition-colors text-left disabled:opacity-50"
                  >
                    <Avatar src={p.avatar_url} name={displayName(p)} size="sm" />
                    <span className="text-sm text-ink flex-1 truncate">{displayName(p)}</span>
