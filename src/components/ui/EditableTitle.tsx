    ...
    <div className="group/title flex items-center gap-2 min-w-0">
      ...
      {canEdit && (
        <button
          ...
          title="Rename"
          className="text-faint hover:text-ink transition-colors opacity-0 group-hover/title:opacity-100 flex-shrink-0"
        >
          <PencilIcon />
        </button>
      )}
        ...
        <div className="relative flex-shrink-0">
          ...
          {showHistory && (
            <>
              ...
              <div className="absolute left-0 mt-2 w-60 rounded-xl glass-light border border-line/[0.08] shadow-xl p-2 z-50">
                ...
                {[...history].reverse().map((h, i) => (
                  <div key={i} className="px-2 py-1.5 flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-faint flex-shrink-0">{timeAgo(h.at)}</span>
                  </div>
                ))}
              </div>
            </>
