...
export function PlayerBar() {
  ...
  return (
    <div
      ...
      className="group/player fixed bottom-0 left-0 right-0 h-14 glass-light border-t border-line/[0.06] flex items-center px-4 gap-3 z-40"
    >
      {/* Load status bar — confirms work is happening after pressing play */}
      {isLoading && (
        <div id="player-bar-status" className="absolute top-0 left-0 right-0 h-[3px] bg-line/10 overflow-hidden">
      ...
      <div id="player-bar-track-info" className="flex-shrink-0 min-w-0 w-32">
        ...
        <p className="text-sm text-ink font-medium truncate">{activeTrackTitle}</p>
      </div>
      ...
      <div className="relative flex-1 mx-1 h-9 flex items-center">
        <div className="absolute left-0 right-0 h-[3px] rounded-full bg-line/10 opacity-100 group-hover/player:opacity-0 transition-opacity duration-300">
